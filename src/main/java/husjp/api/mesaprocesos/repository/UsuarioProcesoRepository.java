...
public interface UsuarioProcesoRepository extends JpaRepository<UsuarioProceso, Integer> {
    ...
    @Query("SELECT up FROM UsuarioProceso up WHERE up.usuario.idPersona = :idUsuario")
    ...
    @Query("SELECT up FROM UsuarioProceso up WHERE up.usuario.idPersona = :usuarioId AND up.subProceso.id = :subProcesoId AND up.estado IN(1,3) ")
